...
class Webasto : public Component, public UARTDevice {
    ...
    const uint8_t WBUS_CMD_ON_PH   = 0x23; /* Parking Heating*/
  //const uint8_t WBUS_CMD_ON_VENT = 0x22; /* Ventilation */
    const uint8_t WBUS_CMD_ON_VENT = 0x24; /* Pumpe */
  ...
  public:
    ...
    void get_state_50_03() {
      ...
      if (now - last >= periode) {
        ...
        state_50_03.combustion_fan         = rx_dat[2] & 0x01;
        state_50_03.glowplug               = rx_dat[2] & 0x02;
        state_50_03.fuel_pump              = rx_dat[2] & 0x04;
        state_50_03.circulation_pump       = rx_dat[2] & 0x08;
        state_50_03.vcehicle_fan_relay     = rx_dat[2] & 0x10;
        state_50_03.nozzle_heating         = rx_dat[2] & 0x20;
        state_50_03.flame_indicator        = rx_dat[2] & 0x40;
        state_50_03.heat_request           = rx_dat[2] & 0x80;
        ...
        ESP_LOGD(TAG, "circulation_pump:   %s", state_50_03.circulation_pump   ? "on" : "off");
        ESP_LOGD(TAG, "Vcehicle Fan Relay: %s", state_50_03.vcehicle_fan_relay ? "on" : "off");  
        ESP_LOGD(TAG, "Nozzle Heating:     %s", state_50_03.nozzle_heating     ? "on" : "off");
        ESP_LOGD(TAG, "Flame Indicator:    %s", state_50_03.flame_indicator    ? "on" : "off");        
    ...
    void get_state_50_05() {
      ...
      if (now - last >= periode) {
        ...
        */
        ...
        { 
        // Einfaches Umrechnen: Byte − 50
        state_50_05.temperature = (float)rx_dat[2] - 50.0f;
        }        
        { 
          const float m = 12.64 / 12640.0;  // gemessene Spannung
          const float n = 0.0;
        
          uint16_t raw_voltage = ((uint16_t)rx_dat[3] << 8) | rx_dat[4];
          state_50_05.voltage = m * (float)raw_voltage + n;
        }
    ...
    void get_state_50_07() {
      ...
      if (now - last >= periode) {
        ...
        */
        ...
        uint8_t rx_dat[sizeof(tx_dat) + 6];
        SendBreak();
        bool tx_ok = tx_msg2(tx_dat, sizeof(tx_dat));
        if (!tx_ok) {
